...
# H_0 = coefficients for sex, SNP, scaled_age, visit * period, visit * sex,
# visit * scaled_age, period * sex, period * age, sex * SNP, sex * age, SNP * age
# are all 0
# H_A = at least one of the above is != 0
# Result is a p-value 0.3971, so cannot reject H_0, and so final.glmm is indeed
# more performant with an AIC of 1767 vs the elaborate model with an AIC of 1780
...
```{r glmm_viz_average_conditional_probability}
...
probs <- predict(final.glmm, 
                 ...
                 re.form = NA, 
                 se.fit = TRUE)
data.fitted.mean$fit <- probs$pred
data.fitted.mean$fit_se <- probs$se.fit
